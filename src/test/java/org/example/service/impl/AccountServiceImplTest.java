...
class AccountServiceImplTest {
    ...
    @Test
    @DisplayName("Thêm mới thành công - Tạo tài khoản và xác nhận đã được lưu")
    void testCreate_AndVerify() {
        // Sinh username duy nhất để tránh đụng dữ liệu giữa các lần chạy test
        String uniqueUser = "test_" + System.currentTimeMillis();
        Account acc = new Account(uniqueUser, "pass123", "STAFF", true);

        // Gọi hàm create - không được ném exception
        assertDoesNotThrow(() -> accountService.create(acc));

        // Xác nhận tài khoản đã thực sự tồn tại trong DB
        assertTrue(accountService.existsByUsername(uniqueUser),
                   "Tài khoản vừa tạo phải tồn tại trong DB");

        // Xác nhận dữ liệu lấy ra khớp với dữ liệu đã tạo
        Account saved = accountService.findByUsername(uniqueUser);
        assertNotNull(saved);
        assertEquals(uniqueUser, saved.getUsername());
